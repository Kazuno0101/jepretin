...
| Frame video digambar SELALU sebagai lapisan dasar canvas di `step()` sebelum `overlay.render()` | Prasyarat perekaman: `canvas.captureStream()` merekam canvas saja — tanpa lapisan video, rekaman hanya berisi efek. Canvas = resolusi video + `object-fit` sama → tampilan tak berubah; tema Bingkai/Blur menimpanya dengan drawImage tersendiri (tak konflik) |
| Tema "Blur": gesture bingkai dua tangan → SELURUH foto blur (`ctx.filter blur` + overscan ±2×blurPx), **tanpa outline/titik sudut**; sub-gaya Ringan/Sedang/Kuat = 0.6%/1.2%/2% lebar canvas | Permintaan user (pilihan B & A): analog tema Bingkai tapi efeknya blur semua area; tanpa garis penghubung — blur saja, efeknya jadi feedback itu sendiri; overscan mencegah garis tepi transparan khas canvas blur; fraksi lebar supaya konsisten lintas resolusi |
| Mode hasil Foto/Video (`MediaMode`, segmented "Mode" di atas tombol "Opsi jepret"): Foto = gesture kepalan → countdown → jepret; Video = tombol "Rekam video" muncul & "Mode jepretan" (1×/3×) disembunyikan; kepalan diabaikan di mode Video; pindah ke Foto saat merekam menyimpan rekaman dulu | Permintaan user: mode harus eksplisit di atas opsi jepret — foto & video tidak bercampur (gesture jepret mati saat merekam); Mode jepretan relevan hanya untuk foto jadi disembunyikan di mode Video (better-layout: progressive disclosure, hanya yang relevan) |
| Rekam video: `useRecorder` — `canvas.captureStream(30)` + `MediaRecorder` (vp9→vp8→webm, 6 Mbps), auto-stop 60s, cancel saat kamera mati; tombol "Rekam video" + badge REC di stage; hasil masuk galeri sebagai kartu video (blob URL lokal, di-revoke saat dihapus), unduh `.webm` tanpa komposit polaroid | Permintaan user (pilihan A): konsisten dengan foto — rekaman diputar/dihapus/diunduh dari galeri yang sama. Blob URL (bukan data URL) supaya memori hemat; auto-stop 60s pengaman; privasi tetap — file tidak pernah keluar perangkat |
